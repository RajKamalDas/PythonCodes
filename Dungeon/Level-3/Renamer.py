...
class Renamer:
    ...
    def UI(self):
        ...
        details = tk.Label(frame, text=f"Rename '{self.extension}'s in:", font=("Bahnschrift", 10))
        detailsName = tk.Label(
            frame,
            text="The Files Will Be Renamed If They Start With:",
            font=("Bahnschrift", 10),
        )
        detailsFind = tk.Label(
            frame,
            text="The Files Will Be Renamed If The Name Has:",
            font=("Bahnschrift", 10),
        )
        detailsRename = tk.Label(frame, text=f"Files Will Be Renamed To:", font=("Bahnschrift", 10))
    ...
    def rename(self):
        try:
            filelist = [
                ...
                if file.endswith(self.extension) and file.startswith(self.oldName.get()) and self.oldNameHas.get() in file
            ]
            for i, file in enumerate(filelist):
                ...
                while os.path.exists(new):
                    new = new.split(".")[0].replace(f"({j-1})", "") + f"({j})" + self.extension
                    j += 1
                os.rename(old, new)
            ...
            self.onceDone.config(fg="Red")
            self.message.set("Directory Beyond Access!")
...
if __name__ == "__main__":
    Renamer(".png", debugPath="Dungeon/Level-3/")
